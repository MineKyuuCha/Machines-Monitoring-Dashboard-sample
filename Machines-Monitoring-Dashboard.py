...
# Visual prototype — database connection can be added later.
# ============================================================
...
# ------------------------------------------------------------
# DEMO DATA
# Replace this section later with database.py / machines.db.
# ------------------------------------------------------------
...
.machine-on {
    border-color: #23783b;
    box-shadow: inset 0 0 0 1px rgba(40,210,70,.05), 0 7px 18px rgba(0,0,0,.20);
}
...
.power-on { color: #39dc4b; text-shadow: 0 0 13px rgba(57,220,75,.20); }
.power-off { color: #ff3944; text-shadow: 0 0 13px rgba(255,57,68,.12); }
...
with st.sidebar:
    ...
    st.markdown('<div class="nav-item nav-active"><span class="nav-icon">▣</span>Dashboard</div>', unsafe_allow_html=True)
    st.markdown('<div class="nav-item"><span class="nav-icon">⚙</span>Machines</div>', unsafe_allow_html=True)
    st.markdown('<div class="nav-item"><span class="nav-icon">▤</span>History</div>', unsafe_allow_html=True)
    st.markdown('<div class="nav-item"><span class="nav-icon">▤</span>Reports</div>', unsafe_allow_html=True)
    st.markdown('<div class="nav-item"><span class="nav-icon">⚙</span>Settings</div>', unsafe_allow_html=True)
...
with chart_col:
    ...
    # Demo cumulative ON-time curves.
    x = list(range(25))
    ...
    for idx, m in enumerate(machines):
        ...
        for hour in x:
            if m["status"] == "ON":
                if hour < (idx + 1) * 2:
                    value += 0.45 + random.random() * 0.15
                else:
                    value += random.choice([0, 0.0, 0.12, 0.2])
            else:
                if hour >= 8 + idx:
                    value += random.choice([0, 0.05, 0.15, 0.25])
                else:
                    value += 0
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="lines",
                name=m["id"],
                line=dict(color=chart_colors[idx], width=2.2, shape="hv"),
                hovertemplate=f"{m['id']}<br>Hour: %{{x}}<br>ON time: %{{y:.1f}} h<extra></extra>",
            )
        )
...
for i, m in enumerate(machines):
    util = utilization(m)
    status_class = "util-text-green" if m["status"] == "ON" else "util-text-red"
    ...
    <tr>
        <td>{m["id"]}</td>
        <td><span class="machine-dot {'dot-on' if m['status']=='ON' else 'dot-off'}"></span>
